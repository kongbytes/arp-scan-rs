...
use arp_scan_lib::scan_options::{OutputFormat, ProfileType, ScanOptions, ScanTiming};
use arp_scan_lib::time::parse_to_milliseconds;
...
const EXAMPLES_HELP: &str = "EXAMPLES:
    ...
    # Launch a default scan on the first working interface
    arp-scan
    ...
    # Launch a scan on a specific interface with a custom IP and stealth profile
...
fn compute_networks(
    ...
    let required_networks: Option<Vec<String>> = list_required_networks(file_value, network_value)?;
...
fn compute_scan_timing(matches: &ArgMatches, profile: &ProfileType) -> ScanTiming {
    match (
        ...
        (None, Some(interval_text)) => parse_to_milliseconds(interval_text)
            .map(ScanTiming::Interval)
            .unwrap_or_else(|err| {
...
pub fn build_scan_options_from_args(matches: &ArgMatches) -> Arc<ScanOptions> {
    ...
    let network_range = compute_networks(file_option, network_option).unwrap_or_else(|err| {
    ...
    let scan_timing: ScanTiming = compute_scan_timing(matches, &profile);
...
mod tests {
    ...
    fn should_have_no_network_default() {
        let networks = compute_networks(None, None);
        assert_eq!(networks, Ok(None));
    }

    #[test]
    fn should_handle_single_ipv4_arg() {
        let networks = compute_networks(None, Some(&"192.168.1.20".to_string()));
    ...
    fn should_handle_multiple_ipv4_arg() {
        let networks = compute_networks(None, Some(&"192.168.1.20,192.168.1.50".to_string()));

        let target_network: Vec<IpNetwork> = vec![
            IpNetwork::V4(Ipv4Network::new(Ipv4Addr::new(192, 168, 1, 20), 32).unwrap()),
            IpNetwork::V4(Ipv4Network::new(Ipv4Addr::new(192, 168, 1, 50), 32).unwrap()),
        ];
    ...
    fn should_handle_single_network_arg() {
        let networks = compute_networks(None, Some(&"192.168.1.0/24".to_string()));
    ...
    fn should_handle_network_mix_arg() {
        let networks = compute_networks(
            None,
            Some(&"192.168.20.1,192.168.1.0/24,192.168.5.4/28".to_string()),
        );

        let target_network: Vec<IpNetwork> = vec![
            IpNetwork::V4(Ipv4Network::new(Ipv4Addr::new(192, 168, 20, 1), 32).unwrap()),
    ...
    fn should_handle_file_input() {
        let networks = compute_networks(Some(&"../data/ip-list.txt".to_string()), None);
    ...
    fn should_fail_incorrect_network() {
        let networks = compute_networks(None, Some(&"500.10.10.10/24".to_string()));

        assert_eq!(
            networks,
            Err("Expected valid IPv4 network range (invalid address: 500.10.10.10/24)".to_string())
        );
    }

    #[test]
    fn should_fail_unreadable_network() {
        let networks = compute_networks(None, Some(&"no-network".to_string()));
